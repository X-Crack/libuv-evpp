...
namespace Evpp
{
    ...
    bool HttpDownloadService::CreaterDownload(EventLoop* loop, CURLM* curl_multi_handler, const String* host, const u32 port)
    {
        return CreaterDownload(loop, curl_easy_init(), curl_multi_handler, std::string(host), port);
    }

    bool HttpDownloadService::CreaterDownload(EventLoop* loop, CURLM* curl_multi_handler, const std::string& host, const u32 port)
    {
        return CreaterDownload(loop, curl_easy_init(), curl_multi_handler, host, port);
    }

    bool HttpDownloadService::CreaterDownload(EventLoop* loop, CURL* curl_easy_handler, CURLM* curl_multi_handler, const std::string& host, const u32 port)
    {
        if (CreaterDownloadSession(curl_easy_handler, host))
        {
            ...
            curl_easy_setopt(curl_easy_handler, CURLOPT_PROGRESSDATA, GetDownloadSession());                                        // 数据传输的对象
            curl_easy_setopt(curl_easy_handler, CURLOPT_PROGRESSFUNCTION, &HttpDownloadSession::DefaultProgress);                   // 进度响应函数
            curl_easy_setopt(curl_easy_handler, CURLOPT_WRITEDATA, GetDownloadSession());
    ...
    bool HttpDownloadService::CreaterDownloadSession(CURL* handler, const std::string& host)
    {
        if (nullptr == http_download_session)
        {
            http_download_session = std::make_shared<HttpDownloadSession>(handler, host);
            return true;
        }
        return false;
    }
